...
class AddrBelongsData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_addr_id = models.CharField(max_length=11)  # This field type is a guess.
    c_belongs_to = models.IntegerField()  # This field type is a guess.
    c_firstyear = models.IntegerField()  # This field type is a guess.
    c_lastyear = models.IntegerField()  # This field type is a guess.
...
class AddrCodes(models.Model):
    c_addr_id = models.CharField(unique=True, primary_key=True, max_length=11)  # This field type is a guess.
...
class Addresses(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_addr_id = models.CharField(blank=True, null=False, max_length=11)  # This field type is a guess.
...
class AltnameCodes(models.Model):
    c_name_type_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class AltnameData(models.Model):
    ...
    c_personid = models.IntegerField()  # This field type is a guess.
    c_alt_name = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_alt_name_chn = models.CharField(max_length=255)  # This field type is a guess.
    c_alt_name_type_code = models.IntegerField()  # This field type is a guess.
    c_source = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
...
class AppointmentTypeCodes(models.Model):
    c_appt_type_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    ...
    check0 = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class AssocCodeTypeRel(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_assoc_code = models.IntegerField()  # This field type is a guess.
    c_assoc_type_id = models.CharField(max_length=255)  # This field type is a guess.
...
class AssocCodes(models.Model):
    c_assoc_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class AssocData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    tts_sysno = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_assoc_code = models.IntegerField()  # This field type is a guess.
    c_personid = models.IntegerField()  # This field type is a guess.
    c_kin_code = models.IntegerField()  # This field type is a guess.
    c_kin_id = models.IntegerField()  # This field type is a guess.
    c_assoc_id = models.IntegerField()  # This field type is a guess.
    c_assoc_kin_code = models.IntegerField()  # This field type is a guess.
    c_assoc_kin_id = models.IntegerField()  # This field type is a guess.
    ...
    c_text_title = models.CharField(max_length=255)  # This field type is a guess.
...
class AssocTypes(models.Model):
    c_assoc_type_id = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class AssumeOfficeCodes(models.Model):
    c_assume_office_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class BiogAddrCodes(models.Model):
    c_addr_type = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class BiogAddrData(models.Model):
    ...
    tts_sysno = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_personid = models.IntegerField()  # This field type is a guess.
    c_addr_id = models.IntegerField()  # This field type is a guess.
    c_addr_type = models.IntegerField()  # This field type is a guess.
    c_sequence = models.IntegerField()  # This field type is a guess.
    c_firstyear = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_lastyear = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_source = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class BiogInstCodes(models.Model):
    c_bi_role_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class BiogInstData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_personid = models.IntegerField()  # This field type is a guess.
    c_inst_name_code = models.IntegerField()  # This field type is a guess.
    c_inst_code = models.IntegerField()  # This field type is a guess.
    c_bi_role_code = models.IntegerField()  # This field type is a guess.
...
class BiogMain(models.Model):
    tts_sysno = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_personid = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    c_name = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_name_chn = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class BiogSourceData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_personid = models.IntegerField()  # This field type is a guess.
    c_textid = models.IntegerField()  # This field type is a guess.
    c_pages = models.CharField(max_length=255)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
...
class ChoronymCodes(models.Model):
    c_choronym_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class Copymissingtables(models.Model):
    id = models.IntegerField(db_column='ID', blank=True, null=False, primary_key=True)  # Field name made lowercase. This field type is a guess.
...
class CountryCodes(models.Model):
    c_country_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class DatabaseLinkCodes(models.Model):
    c_db_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    ...
    c_db_contact_person = models.TextField(db_column='c_db_contact_person', blank=True, null=True)  # Field renamed to remove unsuitable characters. This field type is a guess.
...
class DatabaseLinkData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_person_id = models.IntegerField()  # This field type is a guess.
    c_db_id = models.IntegerField()  # This field type is a guess.
    c_db_sys_id = models.CharField(max_length=255)  # This field type is a guess.
...
class Dynasties(models.Model):
    c_dy = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class EntryCodeTypeRel(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_entry_code = models.IntegerField()  # This field type is a guess.
    c_entry_type = models.CharField(max_length=255)  # This field type is a guess.
...
class EntryCodes(models.Model):
    c_entry_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class EntryData(models.Model):
    ...
    c_assoc_code = models.IntegerField()  # This field type is a guess.
    c_assoc_id = models.IntegerField()  # This field type is a guess.
    c_year = models.IntegerField()  # This field type is a guess.
    c_age = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_nianhao_id = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_entry_nh_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_entry_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_code = models.IntegerField()  # This field type is a guess.
    c_inst_name_code = models.IntegerField()  # This field type is a guess.
    c_exam_field = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_entry_addr_id = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_parental_status = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_attempt_count = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_source = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
...
class EntryTypes(models.Model):
    c_entry_type = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class EthnicityTribeCodes(models.Model):
    c_ethnicity_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class EventCodes(models.Model):
    c_event_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class EventsAddr(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_event_record_id = models.IntegerField()  # This field type is a guess.
    c_personid = models.IntegerField()  # This field type is a guess.
    c_addr_id = models.IntegerField()  # This field type is a guess.
    c_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_nh_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_nh_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_yr_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_intercalary = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_month = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_day = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_day_ganzhi = models.IntegerField(blank=True, null=True)  # This field type is a guess.
...
class ExtantCodes(models.Model):
    c_extant_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class GanzhiCodes(models.Model):
    c_ganzhi_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class HouseholdStatusCodes(models.Model):
    c_household_status_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class KinMourning(models.Model):
    c_kinrel = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class KinshipCodes(models.Model):
    c_kincode = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class LiterarygenreCodes(models.Model):
    c_lit_genre_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class MeasureCodes(models.Model):
    c_measure_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class NianHao(models.Model):
    c_nianhao_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    c_dy = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_dynasty_chn = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class OccasionCodes(models.Model):
    c_occasion_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class OfficeCategories(models.Model):
    c_office_category_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class OfficeCodeTypeRel(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_office_id = models.IntegerField()  # This field type is a guess.
    c_office_type_id = models.CharField(blank=True, null=True, max_length=255)  # This field type is a guess.
    c_office_tree_id = models.CharField(max_length=255)  # This field type is a guess.
...
class OfficeCodes(models.Model):
    tts_sysno = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_office_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    c_dy = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class OfficeTypeTree(models.Model):
    c_office_type_node_id = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class ParentalStatusCodes(models.Model):
    c_parental_status_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class PossessionActCodes(models.Model):
    c_possession_act_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class PossessionAddr(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_possession_record_id = models.IntegerField()  # This field type is a guess.
    c_personid = models.IntegerField()  # This field type is a guess.
    c_addr_id = models.IntegerField()  # This field type is a guess.
...
class PossessionData(models.Model):
    c_personid = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_possession_record_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    c_sequence = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class PostedToAddrData(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    tts_sysno = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_posting_id = models.IntegerField()  # This field type is a guess.
    c_personid = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_office_id = models.IntegerField()  # This field type is a guess.
    c_addr_id = models.IntegerField()  # This field type is a guess.
    c_posting_id_old = models.IntegerField(blank=True, null=True)  # This field type is a guess.
...
class PostedToOfficeData(models.Model):
    ...
    c_posting_id = models.IntegerField()  # This field type is a guess.
    c_posting_id_old = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_sequence = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_firstyear = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_nh_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_nh_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_lastyear = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_nh_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_nh_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_appt_type_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_assume_office_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_name_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_source = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.CharField(blank=True, null=True, max_length=255)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
    c_office_id_backup = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_office_category_id = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_intercalary = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_month = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_intercalary = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_month = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_day = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_day = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_fy_day_gz = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ly_day_gz = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_dy = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_created_by = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_created_date = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_modified_by = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_modified_date = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class PostingData(models.Model):
    c_personid = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_posting_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
    tts_sysno = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class ScholarlytopicCodes(models.Model):
    c_topic_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class SocialInstitutionAddr(models.Model):
    ...
    c_inst_name_code = models.IntegerField()  # This field type is a guess.
    c_inst_code = models.IntegerField()  # This field type is a guess.
    c_inst_addr_type_code = models.IntegerField()  # This field type is a guess.
    c_inst_addr_begin_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_addr_end_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_addr_id = models.IntegerField()  # This field type is a guess.
...
class SocialInstitutionAddrTypes(models.Model):
    c_inst_addr_type_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class SocialInstitutionCodes(models.Model):
    ...
    c_inst_name_code = models.IntegerField()  # This field type is a guess.
    c_inst_code = models.IntegerField()  # This field type is a guess.
    c_inst_type_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_begin_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_by_nianhao_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_by_nianhao_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_by_year_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_begin_dy = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_floruit_dy = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_first_known_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_end_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ey_nianhao_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ey_nianhao_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_ey_year_range = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_end_dy = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_inst_last_known_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_source = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
...
class SocialInstitutionNameCodes(models.Model):
    c_inst_name_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class SocialInstitutionTypes(models.Model):
    c_inst_type_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class StatusCodeTypeRel(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_status_code = models.IntegerField()  # This field type is a guess.
    c_status_type_code = models.CharField(max_length=255)  # This field type is a guess.
...
class StatusCodes(models.Model):
    c_status_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class StatusTypes(models.Model):
    c_status_type_code = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class Tablesfieldschanges(models.Model):
    ...
    change0 = models.TextField(db_column='Change', blank=True, null=True)  # Field name made lowercase. This field type is a guess.
...
class TextBiblcatCodeTypeRel(models.Model):
    id = models.AutoField(primary_key=True)#由于原表有复合主键，这里设置自增主键
    c_text_cat_code = models.IntegerField()  # This field type is a guess.
    c_text_cat_type_id = models.CharField(max_length=255)  # This field type is a guess.
...
class TextBiblcatCodes(models.Model):
    c_text_cat_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class TextBiblcatTypes(models.Model):
    c_text_cat_type_id = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class TextCodes(models.Model):
    tts_sysno = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_textid = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class TextData(models.Model):
    ...
    c_textid = models.IntegerField()  # This field type is a guess.
    c_personid = models.IntegerField()  # This field type is a guess.
    c_role_id = models.IntegerField()  # This field type is a guess.
    c_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_nh_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_nh_year = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_range_code = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_source = models.IntegerField(blank=True, null=True)  # This field type is a guess.
    c_pages = models.TextField(blank=True, null=True)  # This field type is a guess.
    c_notes = models.TextField(blank=True, null=True)
    c_created_by = models.TextField(blank=True, null=True)  # This field type is a guess.
...
class TextRoleCodes(models.Model):
    c_role_id = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
...
class TextType(models.Model):
    c_text_type_code = models.CharField(unique=True, primary_key=True, max_length=255)  # This field type is a guess.
...
class YearRangeCodes(models.Model):
    c_range_code = models.IntegerField(unique=True, primary_key=True)  # This field type is a guess.
