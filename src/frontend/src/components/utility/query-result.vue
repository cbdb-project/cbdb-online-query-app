<template>
  <div ref="wrapper">
    <b-row>
      <b-col>
        <p>
          <span style="color:#E01515">{{ items.length }}</span> results
        </p>
      </b-col>
    </b-row>
    <b-row class="pb-3">
      <b-col>
        <b-button @click="exportData" variant="light">
          <a id="export"></a>Save
          <span v-if="this.selected.length > 0">&nbsp;Selected</span> to GIS
        </b-button>
        <b-button @click="exportID" variant="light">
          <a id="save-person"></a>Save
          <span v-if="this.selected.length > 0">&nbsp;Selected</span> Person IDs
        </b-button>
      </b-col>
      <b-col>
        <b-button-group style="position:absolute;right:1em">
          ...
          >
          <b-button
            v-if="!(items.length === this.selected.length)"
            @click="selectAllRows"
            variant="outline-secondary"
            size="sm"
    ...
    >
      ...
      <template v-slot:cell(NameChn)="data">
        ...
        <span v-if="getSelected.indexOf(data.value) !== -1">{{
          data.selectRow()
        }}</span>
      </template>
    </b-table>
...
export default {
  ...
  methods: {
    ...
    exportID() {
      ...
      link.download =
        "people_list_" + new Date().getTime().toString(36) + ".json";
