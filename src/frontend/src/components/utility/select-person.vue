<template>
  <div>
    <b-button-group>
      <b-button
        v-if="selectFromDb === true"
        variant="outline-primary"
        v-b-modal.select-person
        class="query-condition-button"
        size="sm"
        >{{ $t("globalTerm.selectFromDb") }}</b-button
      >
      <b-button
        v-if="importList === true"
        variant="outline-primary"
        v-b-modal.import-person
        class="query-condition-button"
        size="sm"
        >{{ $t("globalTerm.import") }}</b-button
      >
    </b-button-group>
    <!-- Select People  -->
    <b-modal
      ...
      <b-row>
        <b-col :cols="5" style="text-align:right">
          <b-card>
            <b-form-group
              label-cols="4"
              :label="$t('selectPerson.personName')"
              label-for="select-person-input-name"
            >
              <b-form-input
                id="select-person-input-name"
                :placeholder="$t('globalTerm.cnOrPy')"
            ...
            >
              <b-button
                variant="primary"
                @click="searchPerson(formData.personName)"
        ...
        <b-col :cols="7">
          <b-form-group style="text-align:right">
            ...
            <b-button-group>
              <b-button
                v-if="this.selectedPerson.length > 0"
                @click="clearSelected"
              ...
              >
              <b-button
                v-if="!(this.items.length === this.selectedPerson.length)"
                @click="selectAllRows"
                variant="outline-secondary"
                size="sm"
        ...
        <b-col :cols="5" style="text-align:right">
          <b-card>
            ...
            <b-button
              ...
              variant="primary"
              @click="importPerson"
              :disabled="!(localPL && localPL.length)"
              style="width:96px"
            >
          ...
          <b-alert style="text-align:left" class="mt-3" show>
      ...
      this.axios
        .get(
          "https://cbdb.fas.harvard.edu/cbdbapi/person.php?name=" +
            encodeURI(this.formData.personName) +
            "&o=json"
        )
        .then(
          r => {
