<template>
  <div>
    <b-button
      ...
      v-b-modal="'select-place-table' + name"
      ...
      <b-row>
        <b-col :cols="4" style="text-align:right">
          <b-card>
            <b-form-group
              label-cols="4"
              label="Place Name"
              label-for="select-place-input-name"
            >
              <b-form-input
                id="select-place-input-name"
          ...
          <b-card>
            <b-form-group>
              <b-button
                variant="primary"
                :disabled="isBusy || selectedPlace.length !== 1"
                @click="getSuperiorLoc"
              >
        ...
        <b-col :cols="8">
          <b-form-group style="text-align:right">
            ...
            <b-button-group>
              <b-button
                v-if="this.selectedPlace.length > 0"
                @click="clearSelected"
              ...
              >
              <b-button
                v-if="!(this.items.length === this.selectedPlace.length)"
                @click="selectAllRows"
                variant="outline-secondary"
                size="sm"
...
export default {
  ...
  methods: {
    ...
    handleTableScroll() {
      ...
      if (
        st.$el.scrollHeight - st.$el.scrollTop <= st.$el.clientHeight &&
        vm.isBusy === false
      )
        if (
          vm.result.end !== undefined &&
          vm.result.total !== undefined &&
          vm.result.end < vm.result.total
        ) {
          vm.isBusy = true;
          vm.isBusyLoad = true;
          vm.axios
            .get(vm.result.query + `&start=${vm.result.end + 1}&list=100`)
            .then(
              r => {
