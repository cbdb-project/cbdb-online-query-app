    ...
    <b-collapse :id="genDetails(name, id)">
      <b-row>
        <b-col cols="9">
          ...
          <span v-if="isValidVar(nhCount)"
            >{{ nhCount }} {{ $t("globalTerm.year") }}
          </span>
          <span v-if="isIntc === 'true'"
            >{{ $t("globalTerm.intercalary") }}
          </span>
          <span v-if="isValidVar(month)"
            >{{ month }} {{ $t("globalTerm.month") }}
          </span>
          <span v-if="isValidVar(day)"
            >{{ day }} {{ $t("globalTerm.day") }}
          </span>
        </b-col>
        <b-col cols="3">
          <span v-if="gz"> {{ gz }}{{ $t("globalTerm.ganzhi") }} </span>
        </b-col>
      </b-row>
      <b-row v-if="notes">
        <b-col> {{ $t("globalTerm.notes") }}: {{ notes }} </b-col>
      </b-row>
    </b-collapse>
  </div>
</template>
