            ...
            <b-col cols="8" style="text-align:left">
              ...
              <b-card>
                <b-row class="pl-3" style="text-align:center">
                  <b-col>
                    <span
                      v-if="this.entryTable.length == 0"
                      style="line-height:31px"
                      >**{{ $t("globalTerm.all") }}**</span
                    >
                    <span v-else
                      >{{ entryTable[0]["eNameChn"] }}
                      <span v-if="this.entryTable.length > 1"
                        >及另外{{ this.entryTable.length - 1 }}種入仕途徑</span
                      >
                    </span>
                    <view-selected
                      name="entry"
                      :fields="this.entryField"
                      :items="this.entryTable"
                      @update:items="val => (this.entryTable = val)"
                    ></view-selected>
                  </b-col>
                </b-row>
              </b-card>
